...
public class Parser
{
	...
	private void eatToken(Kind kind)
	{
		...
		{
			System.out.println(this.fname + ": at line " + this.current.lineNum + ": Expect: " + kind.toString());
	...
	private void error()
	{
		System.out.println(this.fname + ": at line " + this.current.lineNum + ": Syntax error: There are some syntax errors near " + this.current.kind.toString());
		System.exit(1);
	}
	...
	private ast.exp.T parseNotExp()
	{
		...
		int lineNum = this.current.lineNum;
		while(this.current.kind == Kind.TOKEN_DOT || this.current.kind == Kind.TOKEN_LBRACK) 
		{
			if(this.current.kind == Kind.TOKEN_DOT) 
			{
				...
				else if(this.current.kind == Kind.TOKEN_ID)
				{
					...
					this.eatToken(Kind.TOKEN_RPAREN);
					ast.exp.Call call = new ast.exp.Call(atomExp, id, args, lineNum);
					atomExp = call;
				}
			} 
			else 
			{
				...
				this.eatToken(Kind.TOKEN_RBRACK);
				ast.exp.ArraySelect arraySelect = new ast.exp.ArraySelect(atomExp, index, lineNum);
				atomExp = arraySelect;
			}
		}
		return atomExp;
	...
	private ast.exp.T parseLtExp()
	{
		...
		while(this.current.kind == Kind.TOKEN_ADD || this.current.kind == Kind.TOKEN_SUB) 
		{
			int lineNum = this.current.lineNum;
	...
	private LinkedList<ast.stm.T> parseStatements()
	{
		...
		while(this.current.kind == Kind.TOKEN_LBRACE || this.current.kind == Kind.TOKEN_IF
		   || this.current.kind == Kind.TOKEN_WHILE
		   || this.current.kind == Kind.TOKEN_SYSTEM || this.current.kind == Kind.TOKEN_ID)
		{
	...
	private LinkedList<ast.dec.T> parseVarDecls()
	{
		...
		while(this.current.kind == Kind.TOKEN_INT || this.current.kind == Kind.TOKEN_BOOLEAN || (this.current.kind == Kind.TOKEN_ID && this.current.isType == true)) 
		{
	...
	private LinkedList<ast.dec.T> parseFormalList()
	{
		...
		String id = null;
		if(this.current.kind == Kind.TOKEN_INT || this.current.kind == Kind.TOKEN_BOOLEAN || this.current.kind == Kind.TOKEN_ID) 
		{
			type = this.parseType();
