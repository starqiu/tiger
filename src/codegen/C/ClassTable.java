...
public class ClassTable 
{
	...
	public void initDecs(String current, LinkedList<codegen.C.dec.T> decs) 
	{
	...
	public void initMethod(String current, codegen.C.type.T ret, 
			LinkedList<codegen.C.dec.T> args, String mid) 
	{
		ClassBinding cb = this.table.get(current);
	...
	public void inherit(String c) 
	{
		...
		LinkedList<Tuple> newFields = new LinkedList<Tuple>();
		newFields.addAll(pb.fields);
		newFields.addAll(cb.fields);
		cb.update(newFields);
		
		// methods
		ArrayList<Ftuple> newMethods = new ArrayList<Ftuple>();
		...
		for(codegen.C.Ftuple t : cb.methods) 
		{
			...
			newMethods.set(index, t);// handle overriding
		}
