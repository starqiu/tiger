...
public class TranslateVisitor implements ast.Visitor {
	...
	public void visit(ast.exp.Call e) {
		...
		// int[]: 1
		// class: 2
		codegen.C.type.T ty = null;
		int num = e.rt.getNum();
		switch (num) {
		case -1:
		case 0:
			ty = new codegen.C.type.Int();
			break;
		case 1:
			ty = new codegen.C.type.IntArray();
			break;
		case 2:
			ty = new codegen.C.type.Class(((ast.type.Class) (e.rt)).id);
			break;
		default:
			break;
		}

		this.exp = new codegen.C.exp.Call(newid, exp, e.id, args, at, ty);
		return;
	}
	...
	public void visit(ast.stm.AssignArray s) {
		...
		this.stm = new codegen.C.stm.AssignArray((codegen.C.exp.Id) id, index,
				exp);
		return;
	}
	...
	public void visit(ast.classs.Class c) {
		...
		this.vtables.add(new codegen.C.vtable.Vtable(c.id, c.gc_map.toString(),
				cb.methods));
		this.classId = c.id;
		...
		for (ast.classs.T c : cs) {
			...
			for (ast.dec.T dec : cc.decs) {
				...
				if (d.type instanceof ast.type.Class
						|| d.type instanceof ast.type.IntArray)
					cc.gc_map.append('1');
				else
					cc.gc_map.append('0');
			}
