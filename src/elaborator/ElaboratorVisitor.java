...
public class ElaboratorVisitor implements ast.Visitor {
	...
	private void error(Kind kind, int lineNum) {
		...
		case ERROR_BOTHBOOL:
			System.out
					.println("the types of both sides of the operator should be boolean");
			break;
		case ERROR_BOTHINT:
			System.out
					.println("the types of both sides of the operator should be int");
			break;
		case ERROR_INT:
	...
	private void warnClass() {
		...
		while (itc.hasNext()) {
			...
			ClassBinding value = (ClassBinding) this.classTable.getTable().get(
					key);
			...
			while (itv.hasNext()) {
				...
				if (!vb.isInitialized && !vb.isUsed)
					System.out.println("warn: at line: " + vb.lineNum
							+ ": the variable \"" + id + "\" is never used");
			}
		}
	}
	...
	private void warnMethod() {
		...
		while (it.hasNext()) {
			...
			VariableBinding value = (VariableBinding) this.methodTable
					.getTable().get(key);

			if (!value.isInitialized && !value.isUsed)
				System.out.println("warn: at line: " + value.lineNum
						+ ": the variable \"" + key + "\" is never used");
		}
	}
	...
	public void visit(ast.exp.And e) {
		...
		if (leftty != null && this.type != null)// if the left and right exist
		{
			if (!leftty.toString().equals("@boolean")
					&& !this.type.toString().equals("@boolean"))
				this.error(Kind.ERROR_BOTHBOOL, e.lineNum);
			else if (!leftty.toString().equals("@boolean")
					|| !this.type.toString().equals("@boolean"))
				this.error(Kind.ERROR_BOTHBOOL, e.lineNum);
		}
			...
			if (leftty instanceof ast.type.Class) {
				...
				} else// if the class exists, then find the method of it
				{
					...
					} else// if the method exists
					{
						...
						for (ast.dec.T d : mty.argsType)// declared arguments of
														// call
						{
							declaredArgsty.add(((ast.dec.Dec) d).type);
						}

						if (declaredArgsty.size() != argsty.size())// if the
																	// number of
																	// arguments
																	// does not
																	// match
						...
						else// if match, then check the types
						{
							for (int i = 0; i < argsty.size(); i++) {
								...
								// judge whether actual is the subclass of
								// declared or not
								if (declared instanceof ast.type.Class
										&& actual instanceof ast.type.Class) {
									String actualStr = actual.toString();
									ClassBinding cb = this.classTable
											.get(actualStr);
									...
									if (!declared.toString().equals(actualStr))
										this.error(Kind.ERROR_TYPEARGS,
												e.lineNum);
								} else {
									if (!declared.toString().equals(
											actual.toString()))// if the types
																// of arguments
																// do not match
										this.error(Kind.ERROR_TYPEARGS,
												e.lineNum);
	...
	public void visit(ast.exp.Id e) {
		...
		if (type == null)// if search failed, then e.id may be a class field
		{
			...
			if (type == null)
				...
				// mark this id as a field id, this fact will be useful in later
				// phase
				e.isField = true;
		...
		if (type != null)// if the id exists
		{
			...
			if (!e.isField)// if the id is the local variable of a method
			{
				...
				if (!this.methodTable.getInit(e.id))
					...
					&& !this.classTable.getUse(className.toString(), e.id))// if
																			// the
																			// id
																			// is
																			// the
	...
	public void visit(ast.stm.Assign s) {
		...
		if (type != null)// if id exists
		{
			...
			if (this.type != null) {
				...
				} else if (s.id.isField
						&& !this.classTable.getUse(className.toString(),
								s.id.id))
	...
	public void visit(ast.method.Method m) {
		...
		if (this.type != null
				&& !this.type.toString().equals(m.retType.toString()))
	...
	private void buildClass(ast.classs.Class c) {
		...
		for (ast.dec.T dec : c.decs) {
			...
			this.classTable.put(c.id, d.id, new VariableBinding(d.type, false,
					d.lineNum));
		}
